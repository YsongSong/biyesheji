            ...
            <c:otherwise>
                <c:forEach var="article" items="${pb.beanList}" varStatus="status">
                    <tr>
                        ...
                        <td>${article.articlecntitle}</td>
                        <td>${article.articlecnkeywords}</td>
                        <td>${article.qikan.qikanname}</td>
                        <td>${article.receivearticletime}</td>
                        <td>
                            <c:if test="${article.editorstatus == 0}">
                                待处理
                            </c:if>
                            <c:if test="${article.editorstatus == 1}">
                                审核中
                            </c:if>
                            <c:if test="${article.editorstatus == 2}">
                                已审核
                            </c:if>
                        </td>
                        <td class="td-manage" style="text-align: center;">
                            <a href="${pageContext.request.contextPath}/download.do?filename=${article.articlepath}&authorId=${article.author.userid}" title="下载">
                                <i class="layui-icon">&#xe601;</i>
                            </a>
                            <a title="预览" onclick="WeAdminShow('预览','${pageContext.request.contextPath}/onlinePreview.do?url=${article.articlepath}&userId=${authorInfo.userid}',600,400)" href="javascript:;">
                                <i class="layui-icon">&#xe60a;</i>
                            </a>
                            <a onclick="modifyArticle(${article.editorstatus},'${pageContext.request.contextPath}/author/modifyArticle.do?articleId=${article.articleid}')" href="javascript:;" title="修改">
                                <i class="layui-icon">&#xe642;</i>
                            </a>

                            <a title="删除" onclick="member_del(${article.editorstatus},'${article.articleid}')" href="javascript:;">
                                <i class="layui-icon">&#xe640;</i>
                            </a>
                        </td>
                    </tr>
...
<script>
    ...
    layui.use(['laydate', 'jquery', 'admin'], function() {
        ...
        /*用户-删除*/
        window.member_del = function (editStatus, id) {
            console.log(editStatus);
            if (editStatus == 1) {
                layer.alert("稿件处于审核状态不允许删除！", {
                    icon: 5
                });
            } else {
                layer.confirm('确认要删除吗？', function(index) {
                    ...
                    $.post(
                        $.ajax({
                            type:"post",
                            url:"${pageContext.request.contextPath}/author/deleteArticleById.do",
                            async:true,
                            dataType: "json",
                            data:{
                                articleId:id,
        ...
        /*文章修改*/
        window.modifyArticle = function (editStatus, url) {
            console.log(editStatus);
            if(!editStatus == 0) {
                layer.msg('稿件处于审核状态不允许被修改!', {
                    icon: 2,
                    time: 2000
                });
            } else {
